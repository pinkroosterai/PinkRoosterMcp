...
public sealed class WorkPackageScaffoldingService(AppDbContext db, IStateCascadeService cascadeService, IEventBroadcaster broadcaster) : IWorkPackageScaffoldingService
{
    ...
    {
        stateChanges ??= [];
        ValidateTaskDependencyIndices(request);
        ...
        return await strategy.ExecuteAsync(async (cancellation) =>
        {
            ...
            var (wp, nextWpNumber) = await CreateWorkPackageEntityAsync(projectId, request, changedBy, stateChanges, cancellation);
            var (phaseEntities, phaseTaskMap, nextPhaseNumber, nextTaskNumber) =
                CreatePhasesAndTasks(wp, request, changedBy);
            var totalDependencies = CreateTaskDependencies(request, phaseTaskMap, wp, stateChanges);

            wp.NextPhaseNumber = nextPhaseNumber;
            wp.NextTaskNumber = nextTaskNumber;

            totalDependencies += await CreateWpDependenciesAsync(wp, request, stateChanges, cancellation);
            ...
            return BuildScaffoldResponse(projectId, nextWpNumber, phaseEntities, phaseTaskMap, totalDependencies, stateChanges);
        }, ct);
    }

    private static void ValidateTaskDependencyIndices(ScaffoldWorkPackageRequest request)
    {
    ...
    private async Task<(WorkPackage wp, int nextWpNumber)> CreateWorkPackageEntityAsync(
        long projectId, ScaffoldWorkPackageRequest request, string changedBy,
        List<StateChangeDto> stateChanges, CancellationToken ct)
    {
        ...
        await CreateEntityLinksAsync(wp, request.LinkedIssueIds, request.LinkedFeatureRequestIds, wpAuditEntries, wpCreateAudit, ct);
        db.WorkPackageAuditLogs.AddRange(wpAuditEntries);

        return (wp, nextWpNumber);
    }

    private (List<WorkPackagePhase> phaseEntities, List<List<WorkPackageTask>> phaseTaskMap, int nextPhaseNumber, int nextTaskNumber)
        CreatePhasesAndTasks(WorkPackage wp, ScaffoldWorkPackageRequest request, string changedBy)
    {
        ...
        return (phaseEntities, phaseTaskMap, nextPhaseNumber, nextTaskNumber);
    }

    private int CreateTaskDependencies(
        ScaffoldWorkPackageRequest request, List<List<WorkPackageTask>> phaseTaskMap,
        WorkPackage wp, List<StateChangeDto> stateChanges)
    {
    ...
    private async Task<int> CreateWpDependenciesAsync(
        WorkPackage wp, ScaffoldWorkPackageRequest request,
        List<StateChangeDto> stateChanges, CancellationToken ct)
    {
        if (request.BlockedByWpIds is not { Count: > 0 })
            return 0;
        ...
        foreach (var blockerWpId in request.BlockedByWpIds)
        {
            var blockerWp = await db.WorkPackages
                .FirstOrDefaultAsync(w => w.Id == blockerWpId, ct)
                ?? throw new InvalidOperationException(
            ...
            if (await cascadeService.HasCircularWpDependencyAsync(wp.Id, blockerWp.Id, ct))
                throw new InvalidOperationException(
    ...
    private static ScaffoldWorkPackageResponse BuildScaffoldResponse(
        long projectId, int nextWpNumber, List<WorkPackagePhase> phaseEntities,
        List<List<WorkPackageTask>> phaseTaskMap, int totalDependencies, List<StateChangeDto> stateChanges)
    {
